...
class BSDateHelper
{
    ...
    public static function getCurrentFiscalYear(){
	    ...
	    FiscalyearModel::where('status','=', 1)->update(['status' => 0]);
	    $current_fiscal_year = \App\Models\FiscalyearModel::where('code', $fiscal_year )->first();
	    $current_fiscal_year->status = 1;
    ...
    public static function getCurrentTrimester(){
        ...
        $c_month = $c_fy[1];
        $trimester = '';
        if($c_month >= 1 && $c_month <= 3){ //baisakh to asadh
            $trimester = 4;
        }
        elseif($c_month >= 4 && $c_month <= 6){ //shrawan to aashwin
            $trimester = 1;
        }
        elseif($c_month >= 7 && $c_month <= 9){ //kartik to poush
            $trimester = 2;
        }
        elseif($c_month >= 10 && $c_month <= 12){ //magh to chaitra
            $trimester = 3;
        }
        if(!empty($trimester)){
            return $trimester;
