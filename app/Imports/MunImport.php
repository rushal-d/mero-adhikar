...
class ContactImport implements ToModel, WithMultipleSheets
{
    ...
    public function model(array $row)
    {
        ...
        //get district id
        $district_name_en_lower = strtolower($district_name_en);
        $district = District::whereRaw('lower(district_name) like (?)', ["{$district_name_en_lower}"])->first();

        $district_id = 0;

        $district_name_np = trim($row[1]);
        if(!empty($district)){
        ...
        $contact =  Contact::create([
            ...
            'phone_en' => trim($row[6]),
            'phone_np' => $this->convertToNepali($row[6]),
        ]);
